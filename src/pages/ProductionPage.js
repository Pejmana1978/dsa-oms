...
export default function ProductionPage({ orders, setOrders, role }) {
  ...
  function buildSheetHTML(o) {
    ...
    const bigThumb = (t) => t ? '<img src="' + t.replace('s-l1600', 's-l500') + '" style="width:150px;height:130px;object-fit:contain;border-radius:4px;border:1px solid #ddd;background:#f9f9f9;display:block" />' : '<div style="width:150px;height:130px;border:1px dashed #ccc;border-radius:4px;background:#f7f7f7"></div>'

    // One block per item, each with its OWN full spec (car/VIN/year/position/
    // material/color/qty) and its own thumbnail (custom override or eBay).
