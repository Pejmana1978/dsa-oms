...
export function printPackingSlip(o) {
  ...
  // Shipped from the Sweden warehouse rather than made to order — the packing
  // team must be able to see this at a glance without opening the OMS.
  const fromStock = !!o.ship_from_stock
  const stockItem = o.stock_item || null
  const stockLine = stockItem
    ? [stockItem.model, stockItem.type, stockItem.colour].filter(Boolean).map(esc).join(' · ')
    : ''

  // Notes are typed in a textarea, so keep the operator's line breaks. esc()
  // first — these are free text and this document is written into a
  // same-origin window.
  const escLines = (s) => esc(s).replace(/\r?\n/g, '<br/>')
  const orderNote = (o.notes || '').trim()
  ...
  const itemRows = items.map(it => {
    ...
    const itemNote = (it.item_notes || '').trim()
    const details = [
      multi ? '' : (allPositions ? `Positions: ${esc(allPositions)}` : ''),
      multi ? '' : (color ? `Color: ${esc(color)}` : ''),
      // Per-item note, against the item it belongs to — on multi-item orders an
      // order-level banner cannot say WHICH item it is about.
      itemNote ? `<span style="display:inline-block;background:#FFFBEB;border:1px solid #F59E0B;border-radius:4px;padding:3px 7px;margin-top:4px;font-weight:bold;color:#92400E">NOTE: ${escLines(itemNote)}</span>` : '',
